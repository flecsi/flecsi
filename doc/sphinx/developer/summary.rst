...
Note that there is no mechanism at present to overlap the ghost copies with a task that does not require access to ghosts or write access to shared points.
...
With Legion, the control model executes in the top-level task, but we reserve the word "task" for the leaf tasks it launches.
Each instance of the replicated top-level task is called a `shard`.
The requirement that the arguments passed to tasks must be identical across shards sometimes necessitates providing extra information from which the point task may select according to its color or else blocking to allow the task to compute it from a source passed via pointer.
