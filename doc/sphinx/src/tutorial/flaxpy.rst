...
The ``initialize_action`` action uses a *coloring slot* to initialize the topology slot defined above in `Control model`_, allocating memory for the ``dist_vector`` specialization.
