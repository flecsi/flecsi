...
struct ntree : ntree_base, with_meta<Policy> {
  ...
  static void set_top_tree_ptrs(field<data::points::Value>::accessor<wo> a,
    const std::vector<std::size_t> & base,
  ...
  void make_tree(typename Policy::slot & ts) {
    ...
    auto fm_sizes =
      flecsi::execute<make_tree_distributed_task>(ts, meta_field(this->meta), top_tree);

    sz.ent.resize(colors());
    sz.node.resize(colors());
    rz.node.resize(colors());
    rz.ent.resize(colors());
    ...
    for(std::size_t i = 0; i < colors(); ++i) {
      rz.ent[i] = sz.ent[i] + top_tree_nents[i];
      rz.node[i] = sz.node[i] + top_tree_nnodes[i];
    }

    // Properly resize the partitions for the new number of ents + ghosts
    part.template get<entities>().resize(make_partial<allocate>(rz.ent));
    part.template get<nodes>().resize(make_partial<allocate>(rz.node));

    cp_top_tree_entities.emplace(
      *this,
      data::copy_plan::Sizes(processes(), 1),
      [&](auto f) { execute<set_destination>(f, sz.ent, top_tree_nents); },
      [&](
        auto f) { execute<set_top_tree_ptrs<entities>>(f, sz.ent, top_tree); },
      util::constant<entities>());

    cp_top_tree_nodes.emplace(
      *this,
      data::copy_plan::Sizes(processes(), 1),
      [&](auto f) { execute<set_destination>(f, sz.node, top_tree_nnodes); },
      [&](auto f) { execute<set_top_tree_ptrs<nodes>>(f, sz.node, top_tree); },
      util::constant<nodes>());
  ...
  void share_ghosts(typename Policy::slot & ts) {
    ...
    // Get current sizes
    auto fm_sizes = flecsi::execute<sizes_task>(meta_field(this->meta));
...
struct ntree<Policy>::access {
  ...
  data::scalar_access<meta_type, ntree::meta_field> mf;
  ...
  void send(F && f) {
    ...
    e_i.topology_send(f);
    n_i.topology_send(f);
    f(mf, [](typename Policy::slot & s) { return meta_field(s->meta); });
  ...
  void reset() {
    ...
    hmap.clear();
    mf.get().max_depth = 0;
    mf.get().top_tree.ents = 0;
    mf.get().ghosts.ents = 0;
    mf.get().nents_recv = 0;
    mf.get().local.nodes = 0;
    mf.get().top_tree.nodes = 0;
    mf.get().ghosts.nodes = 0;
  }
  ...
  auto find_intersect_entities() {
    ...
    std::size_t start = mf.get().local.ents + mf.get().top_tree.ents +
                        mf.get().ghosts.ents;
    std::size_t stop = start + mf.get().nents_recv;
  ...
  auto find_send_entities() {
    ...
    for(std::size_t i = 0; i < mf.get().local.ents; ++i) {
      ent_id id(i);
  ...
  auto entities() {
    if constexpr(PT == ptype_t::exclusive) {
      return make_ids<index_space::entities>(
        util::iota_view<util::id>(0, mf.get().local.ents));
    }
      ...
      return make_ids<index_space::entities>(util::iota_view<util::id>(
        mf.get().local.ents + mf.get().top_tree.ents,
        e_keys.span().size()));
    }
    else {
      // Iterate on all
  ...
  auto nodes() {
    if constexpr(PT == ptype_t::exclusive) {
      return make_ids<index_space::nodes>(
        util::iota_view<util::id>(0, mf.get().local.nodes));
    }
    else if constexpr(PT == ptype_t::ghost) {
      // Ghosts starts from local to end
      return make_ids<index_space::entities>(util::iota_view<util::id>(
        mf.get().local.nodes, n_keys.span().size()));
    }
    else {
      // Iterate on all
      return make_ids<index_space::entities>(
  ...
  void make_tree() {
    ...
    assert(hibound >= e_keys(mf.get().local.ents - 1));
    ...
    {
      std::size_t cnode = mf.get().local.nodes++;
    ...
    for(size_t i = 0; i <= e_keys.span().size(); ++i) {
      ...
      lastnkey = nkey;
      mf.get().max_depth =
        std::max(mf.get().max_depth, current_depth);
  ...
  void load_shared_entity_distant(const std::size_t & c,
    ...
    if(f == hmap.end()) {
      ...
      cur.set_nonlocal();
      cur.set_color(c);
      auto eid = mf.get().local.ents + mf.get().top_tree.ents +
                 mf.get().ghosts.ents++;
      cur.set_ent_idx(eid);
