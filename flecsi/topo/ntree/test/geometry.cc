...
double
rnd(const double & min, const double & max) {
...
geometry_1d_sanity() {
  UNIT {
    ...
    // --------------------------//
    {
      ...
      for(int i = 0; i < 4; ++i) {
        ASSERT_EQ(res_test[i],
          geo1d::intersects_box_box(bmin, bmax, bmin_test[i], bmax_test[i]));
      }
    }
    ...
    // ------------------------//
    {
      ...
      for(int i = 0; i < 5; ++i) {
        ASSERT_EQ(res_test[i],
          geo1d::intersects_sphere_box(
            bmin, bmax, center_test[i], radius_test[i]));
      }
    }
  };
} // geometry_1d_sanity
...
geometry_2d_sanity() {
  UNIT {
    ...
    // --------------------------//
    {
      ...
      for(int i = 0; i < 4; ++i) {
        ASSERT_EQ(res_test[i],
          geo2d::intersects_box_box(bmin, bmax, bmin_test[i], bmax_test[i]));
      }
    }
...
geometry_3d_sanity() {
  UNIT {
    ...
    // -----------------------------//
    {
      ...
      for(int i = 0; i < num_tests; ++i) {
        p3d center(
          rnd(bmin[0], bmax[0]), rnd(bmin[1], bmax[1]), rnd(bmin[2], bmax[2]));
        ASSERT_TRUE(geo3d::within_box(bmin, bmax, center));
      }
      // outside
      for(int i = 0; i < num_tests; ++i) {
        p3d center(rnd(bmin[0] - 10, bmin[0]),
          rnd(bmin[1] - 10, bmin[1]),
          rnd(bmin[2] - 10, bmin[2]));
        ASSERT_TRUE(!geo3d::within_box(bmin, bmax, center));
      }
      for(int i = 0; i < num_tests; ++i) {
        p3d center(rnd(bmax[0], bmax[0] + 10),
          rnd(bmax[1], bmax[1] + 10),
          rnd(bmax[2], bmax[2] + 10));
        ASSERT_TRUE(!geo3d::within_box(bmin, bmax, center));
      }
    }
    ...
    // --------------------------//
    {
      ...
      for(int i = 0; i < 4; ++i) {
        ASSERT_EQ(res_test[i],
          geo3d::intersects_box_box(bmin, bmax, bmin_test[i], bmax_test[i]));
      }
    }
    ...
    // -----------------------//
    {
      ...
      double radius = 1;
      for(int i = 0; i < num_tests; ++i) {
        p3d c = p3d(rnd(center[0] - radius, center[0] + radius),
          rnd(center[0] - radius, center[0] + radius),
          rnd(center[2] - radius, center[2] + radius));
        double r = rnd(radius / 100., radius);
        bool res = util::distance(center, c) <= radius + r;
    ...
    // ------------------------//
    {
      ...
      p3d bmax(1., 1., 1.);
      p3d center_test[5] = {
        {0, 0, 0}, {2, 2, 2}, {3, 3, 3}, {-2, -2, -2}, {2, 2, 2}};
      double radius_test[5] = {1, 1, 1, 1, 5};
      bool res_test[5] = {1, 0, 0, 0, 1};
      for(int i = 0; i < 5; ++i) {
        ASSERT_EQ(res_test[i],
          geo3d::intersects_sphere_box(
            bmin, bmax, center_test[i], radius_test[i]));
      }
    }
  };
