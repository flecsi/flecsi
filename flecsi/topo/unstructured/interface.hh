...
private:
  ...
  util::key_array<int, index_spaces> size_;
...
public:
  ...
  void send(F && f) {
    std::size_t i = 0;
    for(auto & a : size_) {
      data::scalar_access<data::partition::row> tmp_ac;
      f(tmp_ac,
        [&i](typename Policy::slot & u) { return u->part_[i++].sizes(); });
      a = data::partition::row_size(tmp_ac.data());
    }
