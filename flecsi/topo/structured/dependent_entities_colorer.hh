...
auto
dependent_entities_colorer(box_coloring & colored_cells) {
...
void
tag_box_boundary_entities(box_coloring & colored_cells) {
...
void
find_incident_boxes(std::size_t bid,
  box_color & mbox,
  std::vector<box_color> & in_boxes,
  std::vector<int> & inbox_ids) {
  inbox_ids.clear();
    ...
    for(std::size_t s = 0; s < nbids; s++) {
      ...
      for(Dimension d = 0; d < dim; d++) {
        ...
        if((mval == 2) && (inval == 2)) {
          ...
          incident = incident && (mbox.domain.lowerbnd[d] ==
                                   in_boxes[b].domain.lowerbnd[d]);
        }
          ...
          incident = incident && (mbox.domain.upperbnd[d] ==
                                   in_boxes[b].domain.upperbnd[d]);
        }
        else {
          incident = false;
        }
