...
protected:
  ...
  static void copy_sizes_task(topo::resize::Field::accessor<wo> a,
    topo::resize::Field::accessor<ro> b) noexcept {
    a = b;
  } // copy_sizes_task
  ...
  static void init_hist_task(field<hist_int_t>::accessor<wo> v) noexcept {
...
protected:
  ...
  static void histo_task(
    ...
    data::reduction_accessor<exec::fold::sum, hist_int_t> histo) noexcept {
  ...
  static void sort_probes(
    ...
    std::vector<key_type> & sorted_probes) noexcept {
  ...
  static void init_intervals_task(
    ...
    key_type min,
    key_type max) noexcept {
    std::fill(
      intervals.span().begin(), intervals.span().end(), interval{min, max});
  } // init_intervals_task

  static void fake_initialize(typename field<key_type>::template accessor1<
    privilege_repeat<rw, PC>>) noexcept {}
  static void fake_initialize_others(
    field<std::byte, data::raw>::accessor1<privilege_repeat<rw, PC>>) noexcept {
  }
...
public:
  ...
  void operator()() {
    ...
    bool sorted = true;
    const auto fm_check = sched->execute<sort::check_sort_task>(values).all();
    ...
    auto fm_max =
      sched->reduce<sort::reduce_max_meta_task, sort_base::max>(meta_fh);
    sched->execute<sort::init_intervals_task>(
    ...
    for(int i = 0; i < iterations; ++i) {
      ...
      sched->execute<sort::histo_task>(
        values, sort::probes_f(lm_probes), hist_fh);
      // Update bounds on each color
      sched->execute<sort::update_bound_task>(
    ...
    sched->execute<sort::compute_copy_task>(exec::on, transfer_fh, copy_fh);
    ...
    for(auto & af : apply_fields) {
      auto fr = data::field_reference<std::byte, data::raw, topology, space>(
        af->fid, tt);
      sched->execute<sort::fake_initialize_others>(fr);
    }

    // Create copy plan operation and issue
    auto dest = [&](auto f) {
      sched->execute<sort::set_destination_task>(exec::on, f, copy_fh, meta_fh);
    };
    auto src = [&](auto f) {
      sched->execute<sort::set_pointers_task>(exec::on, f, copy_fh, meta_fh);
    };
    {
      data::copy_plan cp(*sched,
        tt,
