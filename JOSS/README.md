# Journal of Open Source Software (JOSS) Paper
## Useful Documentation
- [Submission process](https://joss.readthedocs.io/en/latest/submitting.html)
- [OSI approved license](https://opensource.org/license)
- [Review criteria](https://joss.readthedocs.io/en/latest/review_criteria.html)
- [Paper format](https://joss.readthedocs.io/en/latest/paper.html)
- [Paper example](https://joss.readthedocs.io/en/latest/example_paper.html)

Important note: The paper should be between 250–1000 words.

## Generate the PDF

The steps are provided here: https://joss.readthedocs.io/en/latest/paper.html#checking-that-your-paper-compiles
The easiest solution for me (Julien) was to install `inari` and the font `Hack` locally.
