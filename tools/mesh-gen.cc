...
template<typename T>
void
write(std::ostream & stream, const T & variable) {
...
main(int argc, char ** argv) {
  ...
  std::ios_base::openmode mode =
    ascii ? std::ofstream::out | std::ofstream::binary : std::ofstream::out;
  ...
  tex << "\\draw[step=1cm,black] (0, 0) grid (" << N << ", " << M << ");"
      << std::endl;
